...
Все RLS политики оптимизированы для производительности:

- Используется `(select auth.uid())` вместо `auth.uid()` для кеширования результата на уровне запроса
- Это значительно улучшает производительность при работе с большими таблицами (до 99% улучшения)
- Все политики используют роль `authenticated` для предотвращения ненужных проверок для анонимных пользователей

Подробнее: [Supabase RLS Performance Guide](https://supabase.com/docs/guides/database/postgres/row-level-security#call-functions-with-select)
...
- `invited_by` — для budget_members (добавлен в миграции 20250119000000)
- `to_account_id` — для transactions (добавлен в миграции 20250119000000)

### Оптимизация индексов

Все внешние ключи имеют соответствующие индексы для улучшения производительности JOIN операций и фильтрации. Неиспользуемые индексы удаляются для оптимизации операций INSERT/UPDATE/DELETE.
...
1. **Индексы на внешних ключах**: Все внешние ключи имеют индексы для быстрого поиска и JOIN операций
2. **Оптимизированные RLS политики**: Использование `(select auth.uid())` вместо `auth.uid()` для кеширования
3. **Удаление неиспользуемых индексов**: Регулярная очистка неиспользуемых индексов для оптимизации операций записи

### Миграции производительности

- `20250119000000_add_missing_indexes.sql` - Добавление недостающих индексов
- `20250119000001_optimize_rls_policies.sql` - Оптимизация всех RLS политик
- `20250119000002_remove_unused_index.sql` - Удаление неиспользуемых индексов

### Ожидаемые улучшения

- **Производительность запросов**: Улучшение на 20-30% за счет оптимизации RLS
- **Производительность JOIN**: Улучшение на 50-90% за счет индексов на внешних ключах
- **Производительность записи**: Улучшение на 5-10% за счет удаления неиспользуемых индексов
