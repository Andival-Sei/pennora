...
- **Коммиты:** Conventional Commits на русском языке (см. `docs/processes/commit-convention.md`)
- **Требования к коду:**
...
2. **Определи лучший подход для реализации:**
   - Учти архитектуру проекта (см. `docs/concepts/architecture.md`)
...
1. **Обнови документацию:**
   - Если добавился новый функционал — обнови `docs/meta/mvp-checklist.md` (отметь задачу как выполненную)
   - Если изменилась архитектура — обнови `docs/concepts/architecture.md`
   - Если изменилась база данных — обнови `docs/reference/database.md`
...
1. **Создай коммит на русском языке по соглашению:**
   ...
   - См. `docs/processes/commit-convention.md` для примеров
