...
**Текущее состояние:** 55% покрытия (30+ файлов тестов)
...
- [x] Добавить тесты для `lib/query/mutations/*.ts` — accounts 85%, categories 66%, transactions 38%
- [x] Добавить тесты для `lib/hooks/useSync.ts` — 94% покрытие
- [x] Добавить тесты для `lib/receipt/parser.ts` — 91% покрытие (39 тестов)
- [ ] Интеграционные тесты для auth flow — **перенесено в Фазу 6.2 (E2E тестирование)**
- [x] Настроить CI/CD для запуска тестов при каждом PR
- [x] Создать shared mock utilities (`__tests__/mocks/`) для переиспользования

**Результат:** 55% покрытия достигнуто ✓
...
- [ ] **Auth flow тесты** (перенесено из Фазы 1.2): логин, регистрация, сброс пароля
...
**Последнее обновление:** 2026-01-01 (Фаза 1 завершена: логирование, тесты, типизация)
