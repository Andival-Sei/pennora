...
2. После мерджа PR в `main` автоматически запускается процесс релиза:
   - GitHub Actions workflow анализирует коммиты
   - Определяет новую версию на основе [Conventional Commits](COMMIT_CONVENTION.md)
   - Обновляет `package.json` с новой версией
   - Генерирует `CHANGELOG.md`
   - Создает Git тег (например, `v0.5.0`)
   - Создает GitHub Release с автоматически сгенерированными release notes
   - Коммитит изменения обратно в `main`
...
1. После мерджа PR в `main` запускается GitHub Actions workflow `release`
...
3. На основе типов коммитов (Conventional Commits) определяется новая версия:
   ...
   - `BREAKING CHANGE:` → **major** версия (0.5.0 → 1.0.0)
4. Автоматически обновляется `package.json` с новой версией
5. Генерируется `CHANGELOG.md` с описанием изменений
6. Создается Git тег (например, `v0.5.0`)
7. Создается GitHub Release с автоматически сгенерированными release notes
8. Изменения коммитятся обратно в `main`
