...
Страница статистики (`/dashboard/statistics`) предоставляет визуализацию финансовых данных с поддержкой мультивалютности, фильтрации по периодам и группировки по категориям.

### Компоненты

- **StatisticsPage** — главный компонент с табами (Overview, Trends, Categories)
- **StatisticsFilters** — фильтры периода, типа транзакций и уровня категорий
- **OverviewTab** — обзор с карточками метрик (доходы, расходы, баланс) и круговой диаграммой
- **TrendsTab** — график трендов (Area Chart) с группировкой по дням/неделям/месяцам
- **CategoriesTab** — столбчатая диаграмма и интерактивный список категорий с drill-down
- **CategoryPieChart** — круговая диаграмма расходов по категориям

### Особенности

- **Мультивалютность**: автоматическая конвертация всех сумм в `displayCurrency` пользователя
- **Фильтрация**: по периоду (месяц, 3 месяца, год и т.д.), типу транзакций и уровню категорий
- **Иерархия категорий**: поддержка drill-down для вложенных категорий
- **Сравнение периодов**: процентные изменения относительно предыдущего периода
- **Адаптивность**: все графики адаптивны и оптимизированы для мобильных устройств
- **Анимации**: плавные переходы между табами и состояниями (Framer Motion)

### Архитектура данных

- **lib/query/queries/statistics.ts** — функции агрегации данных с конвертацией валют
- **lib/types/statistics.ts** — типы и константы для статистики
- **lib/utils/category-hierarchy.ts** — утилиты для работы с иерархией категорий

### Оптимизация

- Батч-конвертация валют для уменьшения количества API-запросов
- Мемоизация компонентов для предотвращения лишних ререндеров
- Кеширование запросов через TanStack Query
