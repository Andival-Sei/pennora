...
Приложение использует Background Sync API для автоматической синхронизации данных при восстановлении сети. Это обеспечивает:

- Автоматическую синхронизацию даже когда приложение закрыто
- Надежную доставку операций при нестабильном соединении
- Интеграцию с Service Worker для фоновой работы

**Реализация:**

- Service Worker регистрирует Background Sync события
- При восстановлении сети автоматически запускается синхронизация
- Интеграция с существующей очередью операций через Dexie

## PWA и оффлайн режим

Приложение полностью поддерживает PWA функциональность:

- **Установка**: Промпт установки для Windows и Android
- **Оффлайн страница**: Автоматический fallback при отсутствии сети
- **Кеширование**: Workbox кеширует статические ресурсы и API ответы
